...
- (void)viewDidLoad {
    ...
    [hcts getProjectInfo:1 andUserId:0];
    [hcts setCompletionBlock2:^(NSDictionary *dict) {
        _projectInfo.title = dict[@"title"];
        _projectInfo.introduction = dict[@"introduction"];
        _projectInfo.feature = dict[@"feature"];
        _projectInfo.schedule = dict[@"schedule"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        cell.textLabel.text = _projectInfo.title;
    }else if (indexPath.row == 1)
    {
        cell.textLabel.text = _projectInfo.introduction;
